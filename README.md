...
5. Add these GitHub Actions secrets for Oracle VM deploys: `ORACLE_VM_HOST`, `ORACLE_VM_USER`, `ORACLE_VM_SSH_KEY`, `ORACLE_VM_PORT` (optional), `GHCR_USERNAME`, and `GHCR_TOKEN`.
6. On the Oracle VM, create `/opt/cb-crawler/cb-crawler.env` with the runtime environment variables the container needs, for example `ENV=prod`, database settings, and `PGPASSWORD`.
